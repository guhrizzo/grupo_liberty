// Publica uma release no GitHub (repo guhrizzo/grupo_liberty) com os artefatos
...
const REPO = 'guhrizzo/grupo_liberty'
...
const H = {
  ...
  'User-Agent': 'liberty-car-release',
...
const assets = ['latest.yml', `Liberty-Car-Setup-${version}.exe`, `Liberty-Car-Setup-${version}.exe.blockmap`]
...
if (release) {
  ...
  release = await gh('/releases', {
    method: 'POST',
    body: JSON.stringify({ tag_name: tag, name: `Liberty Car ${version}`, draft: true }),
  })
}
