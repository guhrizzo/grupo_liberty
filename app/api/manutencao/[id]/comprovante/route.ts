...
/**
 * GET /api/manutencao/[id]/comprovante
 *
 * Serve o comprovante (PDF ou imagem) anexado na baixa de uma manutenção.
 * Gate: mesmo acesso de leitura da página /dashboard/manutencao.
 */
export async function GET(
  ...
  const doc = await adminDb.collection('manutencoes').doc(manutencaoId).get()
  if (!doc.exists) {
    return new Response('Manutenção não encontrada.', { status: 404 })
  }

  const comprovante = doc.data()?.baixa?.comprovante as
    | { fileName?: string; contentType?: string; storagePath?: string }
    | null
    | undefined
  if (!comprovante?.storagePath) {
    return new Response('Esta manutenção não tem comprovante anexado.', { status: 404 })
  }
